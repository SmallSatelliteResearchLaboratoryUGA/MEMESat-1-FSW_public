module MessageModule {
    ...
    @
    @ For the convenience of the user, this component specifically stores data of type BBSMessage. 
    @ However, internally, I should abstract from the members
    @ of this type as much as possible. I.e., the component knows that it stores BBSMessages but does not know what members
    @ a BBSMessage has. The only assumption this component really needs to make is that a BBSMessage is a child of 
    @ Serializable. 
    @ In other words, for storing on the satellite, a BBS message does not need to be unpacked from its format.
    @
    @ Thus, this component needs no to minimal change if the definition of the BBSMessage type changes.
    ...
    passive component MessageStorage {
        ...
        @ Stages of writing a BBS message to the file system in which an error can occur   
        enum MessageWriteError { 
            FILE_EXISTS @< A .bbsmsg file with the specified index already exists
        ...
        @ Stages of reading a BBS message from the file system in which an error can occur
        ...
        guarded input port storeMessage: MessageSet 

        @ Load a single stored message by index
        guarded input port loadMessageFromIndex: MessageGetFromIndex
        ...
        @ (also see definition of BBSMessageGetLastN).
        guarded input port loadMessageLastN: MessageGetLastN
        ...
        @ A BBS message has been successfully written to the file system
        ...
        @ An unhandled error occurred while attempting to write a BBS message to the file system
        ...
        @ A BBS message has been successfully read from the file system
        ...
        @ An unhandled error occurred while attempting to read a BBS message from the file system
        ...
        @ 
        @ The component sets the index to 0 if no BBS message was found in the storage directoy.
        @
        ...
        @ The index of BBS messages reached the maximum value of U32 and was thus reset to 0.
        event INDEX_WRAP_AROUND \ 
            severity warning low \ 
            format "Next BBS message index wrapped around from MAX_U32 to 0" \ 
        ...
        event STORAGE_DIRECTORY_WARNING( 
                    directory: string size 128 @< The absolute path of the storage directory which 
                                               @< the BBSMessageStorage component tried to use for 
                                               @< storing messages
            ...
            severity warning low \ 
            format "Storage directory '{}' did not exist. Was component able to fix the error by creating the directory?: {}."
