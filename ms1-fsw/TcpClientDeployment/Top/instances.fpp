module TcpClientDeployment {
  ...
  instance epsUart: EpsModule.EpsUart base id 0x0E00 \
    queue size Default.QUEUE_SIZE \
    stack size Default.STACK_SIZE \
    priority 94

  instance stateMachine: StateMachineModule.StateMachine base id 0x0F00 \
    queue size Default.QUEUE_SIZE \
    stack size Default.STACK_SIZE \
    priority 99

  instance scheduler: SchedulerModule.Scheduler base id 0x1000 \
    queue size Default.QUEUE_SIZE \
    stack size Default.STACK_SIZE \
    priority 100

  instance tlmChanWrapper: TlmChanModule.TlmChanWrapper base id 0x1100 \
    queue size Default.QUEUE_SIZE \
    stack size Default.STACK_SIZE \
    priority 100

  instance fileRecycler: FileRecyclerModule.FileRecycler base id 0x1200 \
    queue size Default.QUEUE_SIZE \
    stack size Default.STACK_SIZE \
    priority 100

  instance otaUpdater: UpdaterModule.OTAUpdater base id 0x1D00 \
    queue size Default.QUEUE_SIZE \
    stack size Default.STACK_SIZE \
    priority 100
  ...
  instance $health: Svc.Health base id 0x2A00 \
    queue size 25
  ...
  instance uartDriver: Drv.LinuxUartDriver base id 0x4C00

  instance messageStorage: MessageModule.MessageStorage base id 0x4D00

  # instance gpio20Drv: Drv.LinuxGpioDriver base id 0x4D00 \
  # {
  #   phase Fpp.ToCpp.Phases.configComponents """ 
  #   { 
  #     const Os::File::Status status = TcpCLientDeplpyment::gpio20Drv.open(
  #         "/dev/gpiochip0", 
  #         20, 
  #         Drv::LinuxGpioDriver::GpioConfiguration::GPIO_INPUT
  #       ); 
  #     if (status != Os::File::Status::OP_OK) { 
  #       Fw::Logger::log("[ERROR] Could not open GPIO 20 driver\\n");
  #     }
  #   }
  #   """
  # }

  # instance gpio21Drv: Drv.LinuxGpioDriver base id 0x4E00 \ 
  # {
  #   phase Fpp.ToCpp.Phases.configComponents """ 
  #   { 
  #     const Os::File::Status status = TcpCLientDeplpyment::gpio21Drv.open(
  #         "/dev/gpiochip0", 
  #         21, 
  #         Drv::LinuxGpioDriver::GpioConfiguration::GPIO_OUTPUT
  #       ); 
  #     if (status != Os::File::Status::OP_OK) { 
  #       Fw::Logger::log("[ERROR] Could not open GPIO 21 driver\\n");
  #     } // if 
  #   } // configComponents
  #   """
  # }

  #instance gpio22Drv: Drv.LinuxGpioDriver base id 0x4F00
  # ..
